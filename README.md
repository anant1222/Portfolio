...
4. **Configure Email Template:**
   ...
   - Use these variables in your email template:
     ...
     - `{{subject}}` - Email subject (required, also use in Subject field)
     - `{{message}}` - Message content (required)
     - `{{time}}` - Timestamp when email was sent (auto-generated)
     - `{{year}}` - Current year (auto-generated)

   **Note**:
   - Make sure the Subject field in your EmailJS template uses `{{subject}}` and not a hardcoded value
   - The template variables use lowercase keys: `{{name}}`, `{{email}}`, etc.
   - `{{time}}` and `{{year}}` are automatically generated by the code
